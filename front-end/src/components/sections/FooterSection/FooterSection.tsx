        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 mb-10">
          <div className="flex flex-col gap-4 max-w-xs">
            ...
            <div className="flex items-center gap-2">
              <a href="#" className="social-btn w-9 h-9 rounded-xl flex items-center justify-center" style={{ color: 'var(--text-1)' }}>
            ...
            {content.footer.columns.map((column, index) => (
              <div key={column.title} className={`flex flex-col gap-3 ${index === 2 ? 'col-span-2 md:col-span-1' : ''}`}>
                ...
                {column.links.map((link) => (
                  <a key={link} href="#" style={{ color: 'var(--text-3)' }} className="hover:text-[#7c6ffa] transition-colors">
                    {link}
                  </a>
                ))}
              </div>
